...
export default function TaskDetailModal({
  ...
  const isGlobalQueue =
    !selectedBoard || selectedBoard.id === 'global' || isPreviewMode;
  const queuePosition = isGlobalQueue
    ? selectedTask.queue_global_number
    : selectedTask.queue_project_number;
  const totalQueue = isGlobalQueue
    ? selectedTask.total_global_queue
    : selectedTask.total_project_queue;
  const mainAssignee = selectedTask.main_assignee;
  const queueLabel = isGlobalQueue
  ...
  const executeSmartNudge = async () => {
    ...
    try {
      ...
      await axios.post(`/api/tasks/${selectedTask.id}/ai-reply`, {
        text: promptText,
      });
      if (showNotification)
        showNotification(
          tMsg(
            'Follow-up sent! It will appear shortly.',
            'Follow-up terkirim! Akan segera muncul.'
          ),
          'success'
        );
    } catch (err) {
      const errorMsg =
        ...
        tMsg('Failed to send Smart Nudge', 'Gagal mengirim pantauan');
  ...
  const regularComments = React.useMemo(
    () =>
      comments.filter(
        (c) => c.username !== 'System' && !c?.text?.startsWith('[ACTIVITY]')
      ),
    [comments]
  );
  ...
  const handleStartTaskMeet = () => {
    ...
    handleAddComment(
      null,
      `@all 🎥 I've started a Google Meet for this task! Join here: ${meetLink}`
    );
  };
  ...
  React.useEffect(() => {
    ...
    sessionLastReadRef.current = localStorage.getItem(
      `innocean_last_read_task_${selectedTask.id}_${currentUser}`
    );
  ...
  React.useEffect(() => {
    if (activeTab === 'comments' && regularComments.length > 0) {
      ...
      const isNewMessageAtBottom =
        !isFirstLoad && lastTaskMsgIdRef.current !== latestMsgId;
      lastTaskMsgIdRef.current = latestMsgId;

      if (!targetId && lastRead && isFirstLoad) {
        const unreadMsg = regularComments.find(
      ...
      if (!initialScrollDoneRef.current) {
        ...
        if (container) {
          const isNearBottom =
            ...
            300;
          const isMyMessage =
            regularComments[regularComments.length - 1].username ===
            currentUser;
  ...
  const isSystemTicket =
    ...
    (selectedTask.category === 'Feedback' ||
      selectedTask.category === 'Support');
  ...
  const generateGoogleCalendarUrl = () => {
    ...
    const startDateStr =
      ...
      (selectedTask.timestamp ? selectedTask.timestamp.split(' ')[0] : '');
  ...
  const handleChatSubmit = (e) => {
    ...
    const isPrivateAI =
      TASK_COMMENT_AI_MENTION_ENABLED && lowerComment.includes('@ai (private)');
    const isTeamAI =
      TASK_COMMENT_AI_MENTION_ENABLED &&
      (lowerComment.includes('@smart assistant') ||
        lowerComment.includes('@ai (team)') ||
      ...
      <div
        className={
          isInline
            ...
            : `bg-white dark:bg-neutral-950 shadow-2xl border border-neutral-200 dark:border-neutral-800 rounded-lg md:rounded-xl w-full max-w-6xl flex flex-col h-[95vh] overflow-hidden transition-all duration-300 ${
        ...
        <div
          className={`flex flex-col flex-1 overflow-hidden min-h-0 ${isInline ? '' : 'lg:flex-row'}`}>
          <div
              ...
              <form
                  ...
                  <div className='space-y-6'>
                    ...
                    <div className='group pt-2'>
                      ...
                      <div className='flex flex-col gap-2 w-full min-w-0'>
                        {(editFormData.supporting_access
                ...
                <div
                  ...
                  <button
                    ...
                    className='w-full sm:w-auto px-6 py-4 sm:py-3 rounded-lg font-bold text-black dark:text-white bg-neutral-100 dark:bg-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 shadow-sm transition-colors uppercase tracking-widest text-xs'>
                    {tMsg('Cancel', 'Batal')}
                  </button>
                  <button
                    type='submit'
                    disabled={isSubtasksLoading || isSubmitting}
                    className={`w-full sm:w-auto px-8 py-4 sm:py-3 rounded-lg font-bold text-white transition-all uppercase tracking-widest text-xs disabled:opacity-50 disabled:cursor-not-allowed shadow-lg border border-black dark:border-white ${
                      isSubmitting
                  ...
                  <div>
                    <div className='flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-3 sm:gap-0 mb-3 pb-3 pt-1 border-b border-neutral-100 dark:border-neutral-800/50'>
                      <div className='flex flex-wrap items-center gap-2 text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-widest min-w-0'>
                            ...
                            if (showNotification)
                              showNotification(
                                tMsg(
                                  ...
                                  'Tautan tugas disalin ke papan klip!'
                                ),
                                'success'
                              );
                          }}>
                        ...
                        <span className='text-neutral-300 dark:text-neutral-700 shrink-0'>
                          /
                        </span>
                        {selectedTask.board_name &&
                          selectedTask.board_name !== 'Unknown' && (
                            <>
                              <span
                          ...
                          <span className='bg-black/80 text-white font-bold px-4 py-2 rounded-xl text-xs uppercase tracking-widest shadow-xl backdrop-blur-md text-center'>
                            ...
                            {currentUser
                              ? tMsg(
                                  ...
                                  'Khusus Anggota Proyek'
                                )
                              : tMsg(
                                  'Login to View Description',
                                  'Login untuk Melihat Deskripsi'
                                )}
                          </span>
                        </div>
                      </div>
                      ...
                      {isTaskAdmin &&
                        ...
                        !isPreviewMode && (
                          <button
                            ...
                            <Icon name='plus' className='w-3.5 h-3.5' />{' '}
                            {tMsg('Add Link', 'Tambah Tautan')}
                          </button>
                        )}
                    </div>
                    {((!selectedTask.supporting_access ||
                      !selectedTask.supporting_access.trim()) &&
                      !isAddingLink) ||
                    isPreviewMode ? (
                      <span className='text-neutral-400 dark:text-neutral-600 font-medium text-sm'>
                        -
                      </span>
                    ) : (
                ...
                <div
                  ...
                  accountStatus !== 'suspended' ? (
                    <button
                      ...
                      className='px-5 py-3.5 w-full sm:w-auto rounded-lg font-bold text-red-500 hover:text-white bg-transparent hover:bg-red-500 border border-transparent hover:border-red-500 transition-all uppercase tracking-widest text-xs'>
                    ...
                    {accountStatus !== 'suspended' &&
                      ...
                      !isPreviewMode && (
                        <button
                          ...
                          className='w-full sm:w-auto px-6 py-4 sm:py-3.5 rounded-lg font-bold text-black dark:text-white bg-neutral-100 dark:bg-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 shadow-sm transition-colors uppercase tracking-widest text-xs'>
                          <Icon name='pencil' className='w-4 h-4 inline mr-1' />{' '}
                          {tMsg('Edit', 'Edit')}
                        </button>
                      )}
                    <button
                      onClick={close}
                      className='w-full sm:w-auto px-8 py-4 sm:py-3.5 rounded-lg font-bold text-white bg-black hover:opacity-80 dark:bg-white dark:text-black border border-black dark:border-white shadow-lg transition-all uppercase tracking-widest text-xs hover:-translate-y-0.5'>
                      {tMsg('Close', 'Tutup')}
                    </button>
                  </div>
                </div>
          ...
          <div
            className={`w-full shrink-0 flex-col h-full z-10 relative bg-cover bg-center ${
              isInline
                ? ''
                : 'lg:w-100 xl:w-112.5 lg:border-l lg:h-auto border-t lg:border-t-0'
              ...
              <div className='flex-1 flex items-center justify-center relative z-20'>
                <div className='z-10 text-center p-6 sm:p-8 bg-white/80 dark:bg-black/80 backdrop-blur-md m-4 sm:m-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-xl'>
                  <div className='mb-6 flex justify-center'>
                    <Icon
                      name='message-circle'
                      className='w-12 h-12 drop-shadow-md text-neutral-400'
                    />
                  </div>
                  ...
                  {currentUser ? (
                    <button
                      ...
                      onClick={() => {
                        axios
                          .post(
                            `/api/boards/${selectedTask.board_id}/request-access?task_id=${selectedTask.id}`
                          )
                          .then((res) => {
                      ...
                      className='bg-indigo-600 text-white font-bold px-8 py-4 rounded-lg text-xs uppercase tracking-widest shadow-lg hover:bg-indigo-700 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:hover:translate-y-0 disabled:cursor-not-allowed'>
                      {hasRequestedAccess
                    ...
                    <button
                      ...
                      className='bg-indigo-600 text-white font-bold px-8 py-4 rounded-lg text-xs uppercase tracking-widest shadow-lg hover:bg-indigo-700 hover:-translate-y-0.5 transition-all'>
              ...
              <>
                    ...
                    {activeTab === 'comments' && (
                      <div className='flex items-center gap-2 px-4 shrink-0'>
                        <button
                          type='button'
                          onClick={() =>
                            setIsTaskChatSearchOpen(!isTaskChatSearchOpen)
                          }
                ...
                {activeTab === 'comments' && isTaskChatSearchOpen && (
                  <div className='relative w-full border-b border-neutral-100 dark:border-neutral-800/50 bg-neutral-50/50 dark:bg-neutral-900/50 p-2 mac-animate shrink-0 z-20'>
                    <Icon
                      name='search'
                      className='absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-neutral-400 pointer-events-none'
                    />
                    <input
                      type='text'
                      placeholder={tMsg(
                    ...
                    {taskChatSearchQuery && (
                      <div className='absolute top-full mt-2 left-2 right-2 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl border border-neutral-200 dark:border-neutral-700 shadow-2xl rounded-xl z-50 max-h-64 overflow-y-auto custom-scrollbar mac-animate'>
                        {regularComments
                          .filter((c) => {
                            const keywords = taskChatSearchQuery
                              .toLowerCase()
                              .split(/\s+/)
                              .filter(Boolean);
                            const combinedText = [c.text, c.username]
                              .join(' ')
                              .toLowerCase();
        ...
        {linkToRemove && (
          <div className='fixed inset-0 bg-white/60 dark:bg-black/60 backdrop-blur-md flex items-center justify-center z-70 p-4 transition-opacity duration-200 opacity-100'>
            <div className='bg-white dark:bg-neutral-950 p-6 sm:p-10 w-full max-w-sm border border-neutral-200 dark:border-neutral-800 shadow-2xl rounded-3xl md:rounded-[2.5rem] text-center mac-animate'>
              <div className='w-20 h-20 bg-red-50 dark:bg-red-900/30 text-red-500 rounded-lg flex items-center justify-center mx-auto mb-6 text-4xl shadow-sm'>
              ...
              <p className='text-neutral-600 dark:text-neutral-400 mb-8 text-sm font-medium leading-relaxed'>
                {tMsg(
                  ...
                  'Apakah Anda yakin ingin menghapus tautan ini?'
                )}
                <br />{' '}
                {tMsg(
              ...
              <div className='flex justify-center gap-4'>
                <button
                  ...
                  className='flex-1 px-4 py-4 rounded-lg font-bold text-black dark:text-white bg-neutral-100 dark:bg-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-colors uppercase tracking-widest text-xs'>
                  {tMsg('Cancel', 'Batal')}
                </button>
                <button
                  onClick={() => {
                  ...
                  className='flex-1 px-4 py-4 rounded-lg font-bold text-white bg-red-500 hover:bg-red-600 shadow-md transition-all uppercase tracking-widest text-xs hover:-translate-y-0.5'>
                  {tMsg('Remove', 'Hapus')}
                </button>
              </div>
            </div>
            ...
            <div className='bg-white dark:bg-neutral-950 p-6 sm:p-10 w-full max-w-sm border border-neutral-200 dark:border-neutral-800 shadow-2xl rounded-3xl md:rounded-[2.5rem] text-center mac-animate'>
              ...
              <div className='flex justify-center gap-3 sm:gap-4'>
                <button
                  ...
                  className='flex-1 px-4 py-4 rounded-lg font-bold text-black dark:text-white bg-neutral-100 dark:bg-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 shadow-sm transition-colors uppercase tracking-widest text-[10px] sm:text-xs'>
                  {tMsg('Cancel', 'Batal')}
                </button>
                <button
                  onClick={executeSmartNudge}
                  disabled={isGeneratingNudge}
                  className='flex-1 px-4 py-4 rounded-lg font-bold text-white bg-amber-500 hover:bg-amber-600 shadow-md transition-all uppercase tracking-widest text-[10px] sm:text-xs hover:-translate-y-0.5 disabled:opacity-50'>
                  {isGeneratingNudge ? (
                    <>
                      <Icon
                        name='clock'
                        className='w-4 h-4 inline animate-pulse mr-1'
                      />
                      ...
                    </>
                  ) : (
