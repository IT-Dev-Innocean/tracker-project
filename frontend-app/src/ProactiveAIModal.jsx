...
export default function ProactiveAIModal({
  ...
  const handleSubmit = async (e) => {
    ...
    setGeneratedTasks([]);
    setLoadingText(
      tMsg('Analyzing your request...', 'Menganalisis permintaan Anda...')
    );
...
"""${prompt}"""`;
      ...
      for (let i = 0; i < extractedTasks.length; i++) {
        ...
        if (extractedTasks[i].suggested_project) {
          const sp = extractedTasks[i].suggested_project
            .replace('#', '')
            .toLowerCase()
            .trim();
        ...
        setGeneratedTasks((prev) => [
          ...prev,
          {
            ...
            deadline:
              extractedTasks[i].deadline && extractedTasks[i].deadline !== ''
                ? extractedTasks[i].deadline
                : '',
            auto_nudge:
              extractedTasks[i].auto_nudge === true ||
              extractedTasks[i].auto_nudge === 'true',
            selected: true,
  ...
  const handleFinish = async () => {
    ...
    for (let i = 0; i < inboxTasks.length; i++) {
      ...
      const formattedData = {
        ...
        subtasks: Array.isArray(t.subtasks)
          ? t.subtasks.map((st) =>
              typeof st === 'string' ? { task_name: st, assignee: null } : st
            )
          : [],
      };
    ...
    if (successCount > 0 && showNotification) {
      showNotification(
        tMsg(
          `Successfully created ${successCount} tasks!`,
          `Berhasil membuat ${successCount} tugas!`
        ),
        'success'
      );
    }
  ...
  return (
    <div
      className={`fixed inset-0 bg-white dark:bg-neutral-950 z-50 overflow-y-auto lg:overflow-hidden font-sans ${
        isClosing ? 'mac-exit' : ''
      }`}>
      <div
            ...
            <div className='w-full max-w-4xl mx-auto flex flex-col flex-1 lg:min-h-0 transition-all duration-500'>
              <div className='shrink-0'>
                <div className='mb-6 lg:mb-8 animate-elegant'>
                  <h1 className='text-2xl sm:text-4xl md:text-5xl font-black text-black dark:text-white leading-tight mb-2 sm:mb-4'>
                    {tMsg(
                      ...
                      'Apa yang ingin Anda capai hari ini?'
                    )}
                  </h1>
                ...
                <form
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      if (mentionMatch) {
                        ...
                        const hasPartialMatch = boardOptions.some((b) =>
                          b.includes(query)
                        );
                    ...
                    onKeyDown={(e) => {
                      if (isMentioning) {
                        const mentionOptions = (userDirectory || [])
                          .filter(
                            (u) => u.is_connected && u.username !== 'admin'
                          )
                          .map((u) => u.username);
                        ...
                        const boardOptions = boards
                          .filter((b) => b.id !== 'global')
                          .map((b) => b.name);
                        const filtered = boardOptions.filter((m) =>
                          m.toLowerCase().includes(boardMentionQuery)
                        );
                        if (e.key === 'ArrowDown') {
                          e.preventDefault();
                          setBoardMentionIndex((prev) => {
                            ...
                            setTimeout(
                              () =>
                                document
                                  .getElementById(`board-item-${nextIdx}`)
                          ...
                          if (filtered.length > 0) {
                            e.preventDefault();
                            insertBoardMention(
                              filtered[boardMentionIndex] || filtered[0]
                            );
                          } else {
                            setIsBoardMentioning(false);
                          }
                  ...
                  {isMentioning && (
                    <div className='absolute left-0 top-full mt-2 w-full min-w-52 bg-white/95 dark:bg-neutral-950/95 backdrop-blur-xl border border-neutral-200 dark:border-neutral-800 shadow-2xl rounded-2xl z-50 max-h-40 overflow-y-auto py-2 mac-animate'>
                      {(() => {
                        const mentionOptions = (userDirectory || [])
                          .filter(
                            (u) => u.is_connected && u.username !== 'admin'
              ...
              {generatedTasks.length > 0 && (
                <div
                  ...
                  <div className='flex justify-between items-center mb-4 shrink-0 px-1'>
                    ...
                    <button
                      onClick={() => {
                        const allSelected = generatedTasks.every(
                          (t) => t.selected
                        );
                        setGeneratedTasks(
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                setGeneratedTasks((prev) =>
                                  prev.map((item) =>
                                    item.id === t.id
                                      ? { ...item, target_board_id: newId }
                                      : item
                                  )
                                );
                              }}
                              className='text-[9px] font-bold bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded text-amber-700 dark:text-amber-400 inline-block uppercase tracking-wider outline-none cursor-pointer max-w-36 truncate [&>option]:bg-white dark:[&>option]:bg-neutral-900 [&>option]:text-black dark:[&>option]:text-white'>
                              {boards
                                .filter((b) => b.id !== 'global')
                                .map((b) => (
                                  <option key={b.id} value={b.id}>
                                    📁 {b.name}{' '}
                                    {b.is_private ? '(Private)' : ''}
                                  </option>
                                ))}
                            </select>
                          </div>
                  ...
                  <div className='sticky bottom-0 mt-4 pt-4 pb-4 lg:pb-0 flex justify-end shrink-0 px-1 z-20 pointer-events-none'>
                    <button
                      ...
                      disabled={
                        isSaving ||
                        isProcessing ||
                        !generatedTasks.some((t) => t.selected)
                      }
                  ...
                  <div
                    ...
                    <div className='flex justify-between items-center mb-2.5 pb-1 pr-6'>
                      <div className='flex items-center gap-1.5 overflow-hidden'>
                        <span className='text-[9px] font-bold text-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 dark:text-indigo-400 px-2 py-1 rounded-md truncate max-w-24 border border-indigo-100 dark:border-indigo-800/50'>
